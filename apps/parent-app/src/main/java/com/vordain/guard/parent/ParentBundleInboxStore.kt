...
data class ParentBundleInboxEntry(
    val id: String,
...
class ParentBundleInboxStore(context: Context) {
    private val preferences = context.getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE)

    fun load(): List<ParentBundleInboxEntry> {
    ...
    fun save(entries: List<ParentBundleInboxEntry>) {
        preferences.edit()
            .putString(KEY_ENTRIES, entries.take(MAX_ENTRIES).joinToString(separator = "\n", transform = ::encodeEntry))
            .apply()
    }

    private fun encodeEntry(entry: ParentBundleInboxEntry): String {
        return listOf(
            encode(entry.id),
    ...
    private fun decodeEntry(line: String): ParentBundleInboxEntry? {
        val parts = line.split('|')
        if (parts.size != FIELD_COUNT) {
            return null
        }
        return runCatching {
            ParentBundleInboxEntry(
                id = decode(parts[0]),
    ...
    companion object {
        private const val PREFERENCES_NAME = "vordain_parent_bundle_inbox"
